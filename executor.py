...
class Executor:
    MAX_BIT_COUNT = 16
    def __init__(self, stack, bit_count):
        assert isinstance(bit_count, int) and bit_count >= 8

        if bit_count >= self.MAX_BIT_COUNT:
            bit_count = self.MAX_BIT_COUNT

        assert 0 == (bit_count & (bit_count - 1)), 'Expected powerOfTwo bit_count'
        self.byte_size = 0xff + 1
        self.word_size = 1 << bit_count
        ...
        def execute_add(cmd):
            self.regs[cmd[1]] += self.get_value(cmd[2])
            self.regs[cmd[1]] %= self.byte_size
...
class Bit8Executor(Executor):
    def __init__(self, *args, **kwargs):
        Executor.__init__(self, *args, bit_count=8, **kwargs)

class Bit16Executor(Executor):
    def __init__(self, *args, **kwargs):
        Executor.__init__(self, *args, bit_count=16, **kwargs)
